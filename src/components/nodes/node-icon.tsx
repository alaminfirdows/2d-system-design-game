...
export const NodeIcon = ({ icon, label, iconClassNames, selected }: NodeProps) => {
    return (
        ...
        >
            <img src={icon} alt={label} className="h-16 w-16 rounded-sm" />

            <p className="absolute right-0.5 bottom-1 left-0.5 z-20 truncate px-1">
                <span className={cn('truncate rounded-sm bg-primary px-2 text-[10px] font-medium text-foreground', iconClassNames)}>{label}</span>
            </p>
        </div>
    );
};
